      ...
      >
        ...
        >
          <div className="p-6 h-full flex flex-col">
            ...
            {/* Links de Navegação Reordenados */}
            <nav className="flex-grow space-y-2">
              ...
              {/* 4. Meus Favoritos */}
              <button onClick={() => handleNav('/favoritos')} className={`w-full flex items-center justify-between p-4 rounded-2xl font-bold transition-all ${isActive('/favoritos') ? 'bg-red-500/10 text-red-500' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-900'}`}>
                <div className="flex items-center space-x-4">
                  <svg className={`w-5 h-5 ${isActive('/favoritos') ? 'fill-current' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                  <span>Meus Favoritos</span>
                </div>
                {favoritesCount > 0 && (
                  <span className="bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-md">{favoritesCount}</span>
                )}
              </button>
