...
import { BottomMenu } from './LayoutBars/BottomMenu';
...
export const Layout: React.FC<LayoutProps> = ({ children, cartCount, favoritesCount, user }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#FFFFFF] dark:bg-[#000000] transition-colors duration-300">
      {/* Sistema de Menu Desktop (Drawer Lateral Direito) */}
      <div 
        className={`fixed inset-0 z-[200] transition-opacity duration-500 hidden lg:block ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      >
        <div className="absolute inset-0 bg-black/40 backdrop-blur-[4px]" onClick={() => setIsMenuOpen(false)}></div>
        
        <aside 
          className={`absolute right-0 top-0 bottom-0 w-[300px] bg-white dark:bg-[#0a0a0a] border-l border-gray-100 dark:border-zinc-800 shadow-2xl transition-transform duration-500 ease-out transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
          <div className="p-8 h-full flex flex-col">
            ...
            <nav className="space-y-4">
              ...
              <button onClick={() => handleNav('/produtos')} className={`w-full flex items-center space-x-4 p-4 rounded-2xl font-bold transition-all ${isActive('/produtos') ? 'bg-brand/10 text-brand' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-900'}`}>
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
                <span>Promoções</span>
              </button>
      ...
      {/* Novo BottomMenu Mobile isolado */}
      <BottomMenu 
        isOpen={isMenuOpen} 
