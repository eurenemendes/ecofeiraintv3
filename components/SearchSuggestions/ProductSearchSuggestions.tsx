...
export const ProductSearchSuggestions: React.FC<ProductSearchSuggestionsProps> = ({
  ...
  // Reutiliza a lógica visual global, mas pode ser customizada para filtros específicos de produtos no futuro
  return (
          ...
          {scannedHistory.length > 0 && (
            <>
              ...
              {scannedHistory.map((code, idx) => (
                <button key={`prod-scan-${idx}`} onClick={() => onSelect(code)} className="w-full flex items-center justify-between p-4 sm:p-6 hover:bg-brand/5 border-b border-gray-50 dark:border-zinc-800/50 group text-left">
          ...
          {recentSearches.length > 0 && (
            <>
              ...
              {recentSearches.map((term, idx) => (
                <button key={`prod-recent-${idx}`} onClick={() => onSelect(term)} className="w-full flex items-center justify-between p-4 sm:p-6 hover:bg-brand/5 border-b border-gray-50 dark:border-zinc-800/50 group text-left">
                  <div className="flex items-center space-x-3 sm:space-x-4">
      ...
      {suggestions.length > 0 && (
        <div className="animate-in fade-in duration-300">
          ...
          {suggestions.map((s, idx) => (
            <button key={`prod-sugg-${idx}`} onClick={() => onSelect(s.label)} className="w-full flex items-center justify-between p-4 sm:p-6 hover:bg-brand/5 border-b border-gray-50 dark:border-zinc-800/50 last:border-none group text-left text-gray-700 dark:text-zinc-200 transition-colors">
              <div className="flex items-center space-x-3 sm:space-x-4">
