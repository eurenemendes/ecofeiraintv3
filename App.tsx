...
const NotFoundState = ({ title, message, buttonText, onAction }: { title: string, message: string, buttonText: string, onAction: () => void }) => (
  <div className="flex flex-col items-center justify-center py-20 sm:py-32 px-4 text-center animate-in fade-in zoom-in-95 duration-500">
    <div className="w-24 h-24 sm:w-32 sm:h-32 bg-white dark:bg-zinc-900 rounded-[2.5rem] flex items-center justify-center mb-10 shadow-inner border dark:border-zinc-800">
      <svg className="w-12 h-12 sm:w-16 sm:h-16 text-gray-300 dark:text-zinc-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <p className="text-gray-500 dark:text-zinc-400 font-medium text-lg max-w-md mb-10">{message}</p>
    <button 
      onClick={onAction}
...
const ProductDetailView = ({ products, stores, favorites, toggleFavorite, addToList }: { 
  ...
  const store = stores.find(s => normalizeString(s.name) === normalizeString(product.supermarket));
  ...
  return (
    <div className="space-y-12 sm:space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-20">
        <div className="bg-white dark:bg-zinc-900 rounded-[3rem] p-6 sm:p-10 flex items-center justify-center border border-gray-100 dark:border-zinc-800 shadow-sm relative group overflow-hidden h-[400px] sm:h-[600px]">
          ...
          {allImages.length > 1 && (
            <>
              <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md p-3 rounded-2xl shadow-lg border border-gray-100 dark:border-zinc-700 hover:scale-110 active:scale-90 transition-all text-gray-400 hover:text-brand"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7" /></svg></button>
              <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md p-3 rounded-2xl shadow-lg border border-gray-100 dark:border-zinc-700 hover:scale-110 active:scale-90 transition-all text-gray-400 hover:text-brand"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7" /></svg></button>
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex space-x-2">
                {allImages.map((_, i) => <button key={i} onClick={() => setActiveImageIndex(i)} className={`h-2 rounded-full transition-all duration-300 ${i === activeImageIndex ? 'bg-brand w-8' : 'bg-gray-200 dark:bg-zinc-700 w-2'}`} />)}
              </div>
            </>
          )}
        ...
        <div className="flex flex-col justify-center space-y-6 sm:space-y-8">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
               ...
               {product.brand && <span className="text-[10px] font-black text-gray-400 bg-gray-100 dark:bg-zinc-800 px-3 py-1.5 rounded-lg uppercase tracking-widest">Marca: {product.brand}</span>}
            </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="w-14 h-14 bg-white dark:bg-zinc-900 rounded-2xl p-2 shadow-sm border border-gray-100 dark:border-zinc-800 flex items-center justify-center overflow-hidden"><img src={store?.logo} className="w-full h-full object-contain" /></div>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Vendido por</p>
              <Link to={`/supermercado/${store?.id}`} className="text-xl font-black text-gray-800 dark:text-zinc-200 hover:text-brand transition-colors">{product.supermarket}</Link>
            </div>
          </div>
          <div className="bg-[#f8fafc] dark:bg-zinc-900 p-10 rounded-[2.5rem] border border-gray-100 dark:border-zinc-800 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button onClick={() => toggleFavorite(product.id)} className={`p-6 rounded-3xl border-2 transition-all flex items-center justify-center hover:scale-105 active:scale-95 ${favorites.includes(product.id) ? 'bg-red-500 border-red-600 text-white shadow-2xl shadow-red-500/30' : 'border-gray-100 dark:border-zinc-800 text-gray-400 hover:border-red-200'}`}><svg className={`w-6 h-6 ${favorites.includes(product.id) ? 'fill-current' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg></button>
          </div>
        </div>
      </div>
      {product.description && (
        <div className="space-y-6">
          <button onClick={() => setIsDescriptionOpen(!isDescriptionOpen)} className="w-full bg-white dark:bg-zinc-900 p-8 sm:p-10 rounded-[2.5rem] border border-gray-100 dark:border-zinc-800 shadow-sm flex items-center justify-between group hover:border-brand transition-all">
          ...
          <div className={`grid transition-all duration-500 ease-in-out ${isDescriptionOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 pointer-events-none'}`}>
            <div className="overflow-hidden bg-white dark:bg-zinc-900 p-8 sm:p-16 rounded-[3rem] border border-gray-100 dark:border-zinc-800">
               <h2 className="text-2xl sm:text-3xl font-black text-[#111827] dark:text-white tracking-tighter mb-6">Descrição do Produto</h2>
               <p className="text-gray-500 dark:text-zinc-400 text-base sm:text-xl font-medium leading-relaxed whitespace-pre-wrap">{product.description}</p>
            </div>
          </div>
        </div>
      )}
        ...
        {comparisons.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
            {comparisons.map((comp, idx) => {
              const compStore = stores.find(s => normalizeString(s.name) === normalizeString(comp.supermarket));
              const compPrice = comp.isPromo ? comp.promoPrice : comp.normalPrice;
              const isOverallCheapest = compPrice <= currentPrice && idx === 0;
              return (
                <div key={comp.id} className={`relative bg-white dark:bg-zinc-900/60 p-6 sm:p-8 rounded-[2rem] border transition-all group overflow-visible flex items-center justify-between hover:shadow-2xl ${isOverallCheapest ? 'border-brand/40 shadow-xl shadow-brand/10 dark:border-brand/30 ring-1 ring-brand/10' : 'border-gray-100 dark:border-zinc-800/60'}`}>
                  ...
                  <div className="flex items-center space-x-6">
                    <div className="w-14 h-14 bg-white dark:bg-zinc-950 rounded-2xl flex-shrink-0 flex items-center justify-center p-2.5 shadow-sm border border-gray-100 dark:border-zinc-800"><img src={compStore?.logo} className="w-full h-full object-contain" /></div>
                    <div>
                      <p className="font-black text-lg sm:text-xl text-gray-800 dark:text-zinc-100 leading-tight">{comp.supermarket}</p>
                      <p className="text-[10px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest mt-1">{compStore?.neighborhood}</p>
                    </div>
                  </div>
          ...
          <div className="p-16 text-center bg-gray-50/50 dark:bg-zinc-900/50 rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-zinc-800"><p className="text-xl font-bold text-gray-400">Não encontramos este item em outras lojas no momento.</p></div>
        )}
      </div>
    </div>
  );
...
const StoreDetailView = ({ products, stores, searchQuery, setSearchQuery, selectedCategory, setSelectedCategory, sortBy, setSortBy, favorites, toggleFavorite, addToList, showSearchSuggestions, setShowSearchSuggestions, searchSuggestionRef, storeCategoriesRef, categories, currentPage, setCurrentPage, onOpenScanner, user }: { products: Product[], stores: Supermarket[], searchQuery: string, setSearchQuery: (q: string) => void, selectedCategory: string, setSelectedCategory: (c: string) => void, sortBy: 'none' | 'price-asc' | 'price-desc', setSortBy: (s: 'none' | 'price-asc' | 'price-desc') => void, favorites: string[], toggleFavorite: (id: string) => void, addToList: (p: Product) => void, showSearchSuggestions: boolean, setShowSearchSuggestions: (b: boolean) => void, searchSuggestionRef: React.RefObject<HTMLDivElement | null>, storeCategoriesRef: React.RefObject<HTMLDivElement | null>, categories: string[], currentPage: number, setCurrentPage: (n: number) => void, onOpenScanner: () => void, user: User | null }) => {
  ...
  return (
    <div className="space-y-12 sm:space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-700">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 sm:gap-12 bg-white dark:bg-zinc-900 rounded-2xl sm:rounded-[3.5rem] p-6 sm:p-16 border border-gray-100 dark:border-zinc-800 shadow-sm relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-12 mt-8 lg:mt-0 w-full lg:w-auto">
            <div className="w-24 h-24 sm:w-44 sm:h-44 bg-[#f8fafc] dark:bg-zinc-950 rounded-xl sm:rounded-[2.8rem] flex items-center justify-center p-4 sm:p-10 border border-gray-100 dark:border-zinc-800 shadow-inner"><img src={currentStore.logo} alt={currentStore.name} className="w-full h-full object-contain" /></div>
            <div className="text-center sm:text-left space-y-4">
              ...
              <div className="flex flex-col items-center sm:items-start space-y-1">
                <p className="text-gray-500 dark:text-zinc-400 font-bold text-xs sm:text-lg flex items-center"><svg className="w-4 h-4 sm:w-5 h-5 mr-2 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>{currentStore.street}, N°{currentStore.number}</p>
                <p className="text-gray-400 dark:text-zinc-500 font-bold text-[10px] sm:text-sm pl-0 sm:pl-7">Bairro: {currentStore.neighborhood}</p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8 sm:space-y-12">
          <div className="flex flex-col space-y-8">
            <div className="flex flex-row items-stretch gap-2 sm:gap-8">
              <div className="relative flex-grow" ref={searchSuggestionRef}>
                <div className="absolute inset-0 bg-gray-100 dark:bg-zinc-800/40 rounded-xl sm:rounded-[2.5rem] -m-1"></div>
          ...
          {paginatedStoreProducts.length > 0 ? (
            ...
            <div className="text-center py-24 sm:py-40 bg-white dark:bg-zinc-900 rounded-2xl sm:rounded-[4rem] border-2 border-dashed border-gray-100 dark:border-zinc-800 flex flex-col items-center px-4">
              <div className="w-20 h-20 sm:w-32 sm:h-32 bg-gray-50 dark:bg-zinc-950 rounded-2xl sm:rounded-[2.5rem] flex items-center justify-center mb-6 sm:mb-10 shadow-inner border dark:border-zinc-800">
                <svg className="w-10 h-10 sm:w-16 sm:h-16 text-gray-300 dark:text-zinc-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
              </div>
              <p className="text-gray-400 dark:text-zinc-500 font-[800] text-xl sm:text-3xl tracking-tight mb-4">Nenhuma oferta encontrada</p>
              <p className="text-gray-400 dark:text-zinc-600 font-bold max-w-md mx-auto">Esta unidade pode não possuir itens para os filtros atuais.</p>
              <button 
                onClick={() => {setSearchQuery(''); setSelectedCategory('Todas');}}
                className="mt-8 text-brand font-black uppercase text-xs tracking-widest underline decoration-2 underline-offset-4"
              >
                Limpar filtros da loja
              </button>
            </div>
          )}
        </div>
...
const App: React.FC = () => {
  ...
  const confirmClearFavorites = () => {
    ...
    if (type === 'products') {
      if (ids) setFavorites(prev => prev.filter(id => !ids.includes(id)));
      else setFavorites([]);
    } else if (type === 'stores') {
      if (ids) setFavoriteStores(prev => prev.filter(id => !ids.includes(id)));
      else setFavoriteStores([]);
    }
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedSupermarket !== 'Todos') result = result.filter(p => normalizeString(p.supermarket) === normalizeString(selectedSupermarket));
  ...
  if (loading || authLoading) return <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-black"><div className="w-16 h-16 border-[6px] border-brand/10 border-t-brand rounded-full animate-spin mb-8"></div><p className="text-gray-500 dark:text-zinc-500 font-[800] text-xl animate-pulse tracking-widest">EcoFeira...</p></div>;
  ...
  return (
    <Layout cartCount={shoppingList.length} favoritesCount={favorites.length + favoriteStores.length} user={user}>
      ...
      {scanErrorMessage && <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[500] bg-brand text-white px-8 py-4 rounded-2xl shadow-2xl font-black text-sm uppercase tracking-widest animate-in slide-in-from-top-4 duration-300"><div className="flex items-center space-x-3"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg><span>{scanErrorMessage}</span></div></div>}
      <Routes>
        <Route path="/" element={
          <div className="space-y-12 sm:space-y-24">
            ...
            <div className="max-w-4xl mx-auto space-y-8 sm:space-y-10 px-4 mb-8 sm:mb-16">
              <div className="relative group" ref={searchSuggestionRef}>
                <div className="absolute inset-0 bg-brand/5 blur-3xl rounded-full scale-90 group-focus-within:scale-100 transition-transform duration-700"></div>
                <div className="relative flex flex-col sm:flex-row bg-white dark:bg-zinc-900 rounded-3xl sm:rounded-[2.5rem] p-3 shadow-2xl border border-gray-100 dark:border-zinc-800 transition-all focus-within:ring-2 focus-within:ring-brand/20">
            ...
            <div className="flex flex-col space-y-6 sm:space-y-10">
              <div className="flex flex-row items-center gap-3 sm:gap-6">
                <div className="relative flex-grow group" ref={searchSuggestionRef}>
                  <div className="relative flex items-center bg-white dark:bg-zinc-900 rounded-xl sm:rounded-[2.5rem] p-1.5 shadow-2xl border border-gray-100 dark:border-zinc-800 transition-all focus-within:ring-2 focus-within:ring-brand/20">
                    <SearchInput 
                      value={searchQuery}
                      onChange={(val: string) => {setSearchQuery(val); setShowSearchSuggestions(true);}}
                      onFocus={() => setShowSearchSuggestions(true)}
